...
const naverHeaders = { "User-Agent": "Mozilla/5.0", "Accept": "application/json", "Accept-Language": "ko-KR,ko;q=0.9", "Referer": "https://m.stock.naver.com/" };
...
const resolveDomesticCode = async (symbol) => {
  ...
  const selected = items.find((item) => item?.nationCode === "KOR" && String(item?.name || "").toLowerCase().replace(/\s+/g, "") === normalized) || items.find((item) => item?.nationCode === "KOR");
...
const resolveUsSymbol = async (symbol) => {
  ...
  if (/^[A-Z0-9.-]+\.[A-Z]+$/.test(raw)) return { ticker: raw.split(".")[0], providerSymbol: raw };
  const items = await searchNaver(raw);
  const selected = items.find((item) => item?.nationCode === "USA" && String(item?.code || "").toUpperCase() === raw) || items.find((item) => item?.nationCode === "USA");
  if (!selected?.code || !selected?.reutersCode) throw new Error("네이버증권에서 미국 종목 코드를 찾지 못했습니다. 종목을 다시 선택해 주세요.");
  return { ticker: String(selected.code).toUpperCase(), providerSymbol: String(selected.reutersCode).toUpperCase() };
};
...
export const handler = async (event) => {
  if (event.httpMethod !== "POST") return { statusCode: 405, body: JSON.stringify({ error: "POST만 지원합니다." }) };
  ...
  try {
    ...
    for (const item of symbols) {
      ...
      try {
        ...
        const result = market === "KR" || market === "ETF_KR" ? await quoteNaverDomestic(symbol) : await quoteNaverUs(symbol);
        quotes.push({
        ...
        errors.push({ instrumentId: item.instrumentId, ticker: item.ticker, message: error instanceof Error ? error.message : "네이버증권 가격 조회 실패" });
      }
    }
    return { statusCode: 200, body: JSON.stringify({ quotes, errors }) };
  } catch (error) {
    return { statusCode: 500, body: JSON.stringify({ error: error instanceof Error ? error.message : "가격 프록시 오류" }) };
  }
};
